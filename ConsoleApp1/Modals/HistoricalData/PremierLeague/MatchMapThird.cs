...
{
    public class MatchMapThird : ClassMap<HistoricalMatchFromCSVFileThird>
    {
        public MatchMapThird()
        {
            Map(m => m.Div);
            Map(m => m.Date).TypeConverter<CustomDateConverter>(); // TypeConverterOption.Format("dd/MM/yyyy");
            Map(m => m.HomeTeam);
            Map(m => m.AwayTeam);
            Map(m => m.FTHG);
            Map(m => m.FTAG);
